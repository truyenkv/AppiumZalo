...
class BetaLibrary(
    ...
    """BetaLibrary is a App testing library for Robot Framework.

    *Locating elements*

    All keywords in BetaLibrary that need to find an element on the app
    ...
    Beta additionally supports some of the _Mobile JSON Wire Protocol_
    (https://code.google.com/p/selenium/source/browse/spec-draft.md?repo=mobile) locator strategies
    It is also possible to specify the approach BetaLibrary should take
    ...
    def __init__(self, timeout=5, run_on_failure='Capture Page Screenshot'):
        """BetaLibrary can be imported with optional arguments.

        `timeout` is the default timeout used to wait for all waiting actions.
        It can be later set with `Set Beta Timeout`.

        `run_on_failure` specifies the name of a keyword (from any available
        libraries) to execute when a BetaLibrary keyword fails. By default
        ...
        Examples:
        | Library | BetaLibrary | 10 | # Sets default timeout to 10 seconds                                                                             |
        | Library | BetaLibrary | timeout=10 | run_on_failure=No Operation | # Sets default timeout to 10 seconds and does nothing on failure           |
        """
        for base in BetaLibrary.__bases__:
            base.__init__(self)
